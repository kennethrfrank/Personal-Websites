var arrSections = ['merch', 'music', 'events'];
var arrIMG = ['JL', 'K', 'JK'];
var arrTitles = ['', 
	"", 
	""];
...
var backgrounds = ['#600000', '#600000', '#600000' ];//D33762
var backgrounds0 = ['#B8D3AF', '#B8D3AF', '#B8D3AF' ];
var backgrounds1 = ['#3F6037','#3F6037','#3F6037'];
var numerical = ['0', '1','2'];
var generals = ['ABOUT', 'CONTACT', 'MEDIA'];
var socials= ['Instagram','Twitter','Facebook','Youtube'];
var carousels = ['JohnRose Sounds',' ',' ',' ', ' '];
var videos = [];
var contents = [];
			...
			var ndx;
					...
					$('#' + x).on('mouseenter', '.btn', function(button){
						...
						var LWP = numerical[ndx];
							...
							for (i=0; i<=2; i++){
							$('#articleContainer'+i).slideDown(3000);}
							$('#carousel').slideUp(2500);
					...
					$('#' + x).on('mouseleave', '.btn', function(button){
						...
						for (i=0; i<=2; i++){
						$('#articleContainer'+i).slideUp(3000);}
						$('#carousel').slideDown(2500);
...
function buildCarousel(){
	...
	string += `<div class="item active carouselLogo col-xs-12">
        		  <img id="galleryItem" src="./imgs/carousel/photo_0.jpg" alt="`+carousels[0]+`" >
	...
	for (var i = 1; i<carousels.length; i++){
		string += `<div class="item col-xs-12">
...
function buildBox(){
	...
	for(var i = 0; i < arrSections.length; i++){
		...
		string +='}</style>';
			...
			string +=	 '                <img class="columnTitleImg" src = "./badges/'+arrSections[i]+'.png" style ="width: 70%; height: 94.5%;" >';
		...
		string +=   '<div id = "articleContainer'+i+'" class ="'+arrSections[i]+'" >';
		...
		if (i == 1){
	        ...
	        string +=   '<iframe width="90%" height="400" style = "margin-top: 5%" scrolling="no" frameborder="no" src="https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/users/189366782&amp;auto_play=true&amp;hide_related=false&amp;show_comments=true&amp;show_user=true&amp;show_reposts=false&amp;visual=true"></iframe>';
	    ...
	    if (i == 2){
	    	...
	    	previous = ['July 1st: The Vault    FRISCO TX', 'July 4th: Freedom Fest    FRISCO TX'];
	    	upcoming = ['July 30th: The Rail Yard   FRISCO, TX'];

	    	for (var encompassingEvents = 1; encompassingEvents <=2; encompassingEvents++){



	    	string += `<h4 style = "font-size: 30px; font-weight: bold;">
	    					`; 

	    					string += eventType[encompassingEvents-1];
	    		...
	    		if (encompassingEvents == 1){
	    		events = previous;
	    		}
	    		else if (encompassingEvents == 2){
	    		events = upcoming;
	    		}
	    	for (var eve = 0; eve < events.length; eve++){
	    		string += '<div id = "pEvents">';
	    	string += events[eve];
	    		string += '</div>';
	    	string += "<br>";
	    	}
	...
	if (i == 0){
	       string += `<h3 style = "font-size: 50px; font-weight: bold; color: #fafafa;">
	       			UNDER 
	       				</h3>
	       				<h3 style = "font-size: 50px; font-weight: bold; color: #fafafa;">
	       				CONSTRUCTION
	       				</h3>
	       		` ;
	...
	var string = " "
	 string +=	'<div class = "row">';
     		...
     		string +=	'          <div class = "col-xs-6" style="font-weight: bold; font-size: 20px; margin-top: 15px">';
  			string += '&copy; JohnRose &amp; Kenneth Frank';
			string +=	'               </div>';
